...
New to hunting? Follow the [Bug Bounty Hunting Methodology](https://sudoninja-noob.github.io/tool/Bug_Bounty_Hunting_Methodology.html) – also one click away from the popup's **Methodology** button.
...
- **Methodology** – the popup links to the [Bug Bounty Hunting Methodology](https://sudoninja-noob.github.io/tool/Bug_Bounty_Hunting_Methodology.html) guide.
...
- Nothing else is sent anywhere unless you turn on **Also scan Google search results** – then BountyRadar requests up to 40 result pages per search from your browser, so those sites see a visit from your IP even though you never opened them.
...
- **Methodology** – opens the [Bug Bounty Hunting Methodology](https://sudoninja-noob.github.io/tool/Bug_Bounty_Hunting_Methodology.html) guide in a new tab.
...
3. Important: after any change to `manifest.json`, clicking reload re-registers content scripts. Refresh any already-open tabs once so they get the latest content script.
...
| `popup.js`       | Popup logic (search, clear, dorks, site card, scope copy)           |
